...
sum(nnt) as nnt,
sum(cm1) as cm1,
quantitySales as Ilosc,
sum(movingAvgPrice) as GLD,
movingAverageCOGS as COGS,+
shippingCostsNnt as ShipCostNNT,
processLogisticsCosts as processLogisticsCost,
processCostsTotal as processCostTot
...
group by customerNo_key, invoiceNo_key, orderNo_key, fiscalYear, period, productNo, insert_key,  shippingCondition_key,termsOfPayment, discount, supplierDiscount, catalogPriceNet, quantitySales, COGS, ShipCostNNT, processLogisticsCost, processCostTot
order by customerNo_key, invoiceNo_key, orderNo_key, fiscalYear, period, productNo, insert_key,  shippingCondition_key,termsOfPayment,   discount, supplierDiscount, catalogPriceNet, quantitySales,COGS, ShipCostNNT, processLogisticsCost, processCostTot