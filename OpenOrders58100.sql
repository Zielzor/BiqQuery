CREATE OR REPLACE TABLE `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step1` as 
SELECT DISTINCT 
a.customerNo_key,
a.orderNo_key,
...
--------
CREATE OR REPLACE TABLE `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step2` as 
SELECT
a.*,
b.salesDistrict 

FROM `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step1` as a
LEFT JOIN `conrad-cbdp-prod-core.de_conrad_dwh1000_dwh_DimCustomer.DimCustomer` as b on a.customerNo_key = b.customerNo_key;
-------------
CREATE OR REPLACE TABLE `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step3` as 
SELECT
a.*,
b.period as Real_period

FROM `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step2` as a
LEFT JOIN `conrad-cbdp-prod-core.de_conrad_dwh1000_dwh_DimCalendar.DimCalendar` as b on a.dateDisposition_key = b.date_key;

-----------
CREATE OR REPLACE TABLE `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step4` as 
SELECT
a.*,
b.maingrpDescEng as Head_Group,
b.categoryDescEng as Categorie,
b.category_key as category_key, 
FROM `ceeregion-prod.FactOpenOrdersEast.OpenOrders_Step3` as a 
